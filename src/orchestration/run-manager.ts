  ...
  startTask(input: {
    ...
    if (input.kind === "fix") {
      ...
      if (!run.currentTaskId) {
        throw new RunManagerError(
          "CURRENT_TASK_REQUIRED",
          "A fix requires a current reviewed task."
        );
      }
      ...
      if (
        ...
        throw new RunManagerError(
          "INVALID_ROOT_TASK",
          "The requested fix does not belong to the current task chain."
        );
      }
    ...
    void this.runner({
      ...
      .then((result) => {
        ...
        currentRun.currentTaskId =
          taskId;

        const item =
          currentRun.items.find(
            (candidate) =>
              candidate.id ===
              current.planItemId
          );
